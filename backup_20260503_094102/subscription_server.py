...
_EN_MAP = {
    ...
    "germany": "🇩🇪 Германия", "frankfurt": "🇩🇪 Германия",
    "france": "🇫🇷 Франция", "paris": "🇫🇷 Франция",
    "netherlands": "🇳🇱 Нидерланды", "amsterdam": "🇳🇱 Нидерланды",
    "finland": "🇫🇮 Финляндия", "sweden": "🇸🇪 Швеция",
    "poland": "🇵🇱 Польша", "united states": "🇺🇸 США", "miami": "🇺🇸 США",
...
def _rename_marzban(cfg, index):
    """Переименовываем Marzban конфиги — основные серверы.
    ИСПРАВЛЕНО: используем только URL-encode (не двойной encode).
    """
    cfg = cfg.strip()
    ...
    # quote safe="" — кодируем всё кроме unreserved символов
    return base + "#" + urllib.parse.quote(label, safe="")


async def _fetch_marzban(sub_url):
    """Получаем конфиги из Marzban по токену подписки."""
    if not sub_url:
        return []
    # sub_url = "https://panel.komissarovka.ru/sub/user_XXX" — берём только токен
...
async def handle_subscription(request: web.Request) -> web.Response:
    ...
    if not sub or (sub.expires_at and sub.expires_at < datetime.utcnow()):
        label = urllib.parse.quote("Подписка закончилась — купите в @VpnBlay_bot", safe="")
