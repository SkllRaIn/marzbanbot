...
Логика:
  ...
  5. Ночью (03:00 UTC) пул перепроверяется автоматически
"""
...
# Российские серверы-маркеры (мобильная/LTE)
...
def _extract_label(config_line: str) -> str:
    """Извлекает метку после # из vless-конфига, декодирует URL-encode."""
    try:
        if "#" in config_line:
...
def _make_pretty_label(label: str, index: int, expires_at=None) -> str:
    """
    Красивое переименование без лишних символов.
    Пример: 🇷🇺 Россия 1  |  🇩🇪 Германия 1
    """
    ...
    if country_name and "🇷🇺 Россия" in country_name:
        if any(m in label_lower for m in RU_LTE_MARKERS):
            country_name = "🇷🇺 Мобильная LTE"
...
def _set_label(config_line: str, new_label: str) -> str:
    ...
    ИСПРАВЛЕНО: safe="" — кодируем всё, включая эмодзи, чтобы v2ray не дублировал.
    """
...
async def get_pool_configs(count_ru: int = 5, count_foreign: int = 5, expires_at=None) -> list:
    ...
    Конфиги переименовываются «на лету».
    """
...
async def recheck_pool_nightly():
    """
    Ночная перепроверка всего пула (вызывается из scheduler в 03:00 UTC).
    Перезагружает pool.txt, заново проверяет все конфиги и обновляет Redis.
    """
    import os
    pool_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'pool.txt')
    if not os.path.exists(pool_path):
        logger.warning("pool.txt not found for nightly recheck")
        return

    logger.info("Starting nightly pool recheck...")
    with open(pool_path) as f:
        text = f.read()

    result = await process_pool_file(text)
    logger.info(f"Nightly pool recheck done: {result}")
    return result
